...
/*
FrontServlet이 넘겨준 요청 path를 가지고 실제 실행할 클래스의 객체를 만들어서  return하는 작업을 수행
...
public class RequestMapping {
	...
	public Action mapping(String path) {
		//매개변수로 전달된  path로 등록된 Action객체를 actionMap에서 찾아서 리턴하는 코드가 정의되어야 하나
		//우리는 if... else if로 우선 처리
		//현재 코드는 연습용이므로 추가가 되면 mapping메소드를 변경해야 하지만 실제는 xml문서나 properties파일을 분석해서 
		//자동으로 만들도록 코드를 정의할 것이므로 코드 변경을 하지 않아도 된다.
		Action action = null;
